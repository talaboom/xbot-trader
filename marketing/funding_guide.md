# X Bot Trader — Funding & Sponsorship Guide

## YOUR PROFILE
- 45 years old, Canadian + Bulgarian citizen (EU passport)
- University student (but doesn't qualify for student-specific discounts/programs)
- Has a house and car (disqualifies from some need-based programs)
- Bank accounts in Canada + Bulgaria
- Live SaaS product deployed and working
- Looking for: grants, investors, sponsors — NO age caps, NO student-only

---

## IMMEDIATE (This Week — $0 Cost)

### 1. Canadian Funding (No Age Caps)

**BDC (Business Development Bank of Canada)** — bdc.ca
- Startup loans $25K-100K+ — NO age limit
- They specifically fund tech startups
- Lower rates than banks
- Apply online

**IRAP (NRC Industrial Research Assistance Program)** — nrc.canada.ca/irap
- Up to $50K for tech R&D — NO age limit
- Your AI/ML trading bot development qualifies
- Free business advisors included
- One of the best Canadian programs

**SR&ED Tax Credits (CRA)**
- Get 35% of your development costs back as a tax refund
- All the coding you did building X Bot Trader qualifies
- File with your taxes — can be retroactive
- Could be worth $10K-30K depending on hours spent

**Canada Small Business Financing Program**
- Government-backed loans up to $350K
- Any Canadian business owner, no age limit

**CanExport Innovation** — tradecommissioner.gc.ca
- Up to $75K to help Canadian tech expand internationally
- Your EU citizenship + Bulgarian bank = international angle

**Canada Digital Adoption Program (CDAP)**
- Up to $15,000 grant for digital businesses
- No repayment required

**Innovative Solutions Canada (ISC)**
- Government grants for tech companies
- $10K-150K depending on stage

### 2. EU / Bulgarian Funding (Your EU Passport Unlocks These)

**EU Horizon Europe — EIC Accelerator**
- Up to €2.5 MILLION for tech startups
- Any EU citizen can apply
- Your Bulgarian passport qualifies you
- AI + fintech is a priority area

**Bulgaria Innovation Fund**
- Grants for Bulgarian tech entrepreneurs
- Apply through your Bulgarian bank/business registration

**Startup Bulgaria** — startupbulgaria.com
- Grants up to €50K for Bulgarian founders
- Sofia has a growing fintech ecosystem

**EU SME Instrument**
- €50K for feasibility, up to €2.5M for scaling
- For small businesses in EU member states

**JEREMIE Bulgaria**
- EU-backed venture funding for Bulgarian startups
- Equity investments for early-stage companies

**How to access EU funding:**
1. Register a company in Bulgaria (cheap and fast — can do online)
2. Apply to EU grants as a Bulgarian company
3. Keep your Canadian company too — dual presence
4. This gives you access to BOTH Canadian and EU funding pools

### 3. Startup Accelerators (FREE — they invest in you)
Apply to all of these NOW:

**Y Combinator** — ycombinator.com
- $500K for 7% equity
- Best accelerator in the world
- Next batch application deadline: check site
- Your pitch: "AI crypto trading for beginners, live product, $20/mo SaaS"

**Techstars** — techstars.com
- $120K for 6% equity
- Multiple programs (Fintech, AI)
- Strong mentor network

**Creative Destruction Lab (CDL)** — creativedestructionlab.com
- Canadian, based in Toronto
- Perfect for AI/fintech startups
- No equity taken upfront

**DMZ (Toronto Metropolitan University)**
- Free incubator, no equity
- Office space, mentoring, connections

**Communitech (Waterloo)**
- Free programs for early-stage startups

**FoundersBeta** — foundersbeta.com
- $25K investment
- Remote-friendly accelerator

### 3. Angel Investors
Find them here:

**AngelList** — angel.co
- Create a startup profile
- Post your raise ($150K pre-seed)
- Angels can find and invest

**National Angel Capital Organization (NACO)** — nacocanada.com
- Canadian angel network
- They connect startups with investors

**LinkedIn**
- Search: "angel investor crypto Canada"
- Send a short DM with your pitch link

### 4. Crypto-Specific Funding

**Gitcoin Grants** — gitcoin.co
- Crypto community funding
- Open source projects get funded by the community

**Coinbase Ventures** — coinbase.com/ventures
- They invest in crypto startups
- Your app uses Coinbase API — natural fit

**Binance Labs** — labs.binance.com
- Crypto accelerator + investment

**Solana Foundation Grants**
- You accept SOL payments — apply for a grant

**Ethereum Foundation Grants**
- For projects building on crypto ecosystem

### 5. Crowdfunding

**Kickstarter/Indiegogo**
- Pitch: "AI crypto trading app — back us, get lifetime access"
- Reward tiers: $20 = 1 year free, $50 = lifetime, $200 = lifetime + Pro

**Republic.co**
- Equity crowdfunding
- Anyone can invest $50+
- You keep control

**WeFunder** — wefunder.com
- Same as Republic
- Good for consumer-facing products

---

## SPONSORSHIP OPPORTUNITIES

### Crypto Companies That Sponsor
Reach out to these — they sponsor crypto content creators:

1. **Coinbase** — affiliate program pays per signup
2. **Binance** — sponsor crypto educators
3. **Ledger** — sponsor crypto content
4. **BlockFi / Celsius successors** — sponsor fintech content
5. **TradingView** — sponsor trading content creators

### How to Get Sponsored
1. Build an audience first (even 1,000 followers)
2. Create a media kit (I can make this)
3. DM brands: "I have X followers interested in crypto trading. I built an AI trading app. Want to sponsor my content?"
4. Start with affiliate links, then negotiate flat-rate sponsorships

---

## ACTION PLAN — Do This Today

### Priority 1: Free Money (Applications)
- [ ] Apply to Futurpreneur Canada
- [ ] Apply to Y Combinator
- [ ] Apply to Creative Destruction Lab
- [ ] Create AngelList profile
- [ ] Apply to Coinbase Ventures

### Priority 2: Get Users (Needed for investor proof)
- [ ] Post first TikTok/Reel (screen record the app)
- [ ] Post on r/cryptocurrency and r/algotrading
- [ ] Share on Twitter/X with crypto hashtags
- [ ] Tell friends to sign up (use referral link)

### Priority 3: Revenue (Shows investors you're real)
- [ ] Get 10 paying users at $20/mo = $200 MRR
- [ ] Screenshot the dashboard showing real users
- [ ] This is more powerful than any pitch deck

---

## YOUR PITCH (Copy-Paste for Emails/DMs)

**Subject: AI Crypto Trading App — Live Product, Seeking Pre-Seed**

Hi [Name],

I built X Bot Trader — an AI-powered crypto trading platform that lets beginners trade automatically with bots on their Coinbase account.

The product is live at aix.8bit.ca with:
- Paper trading ($100K virtual money)
- 4 AI bot strategies
- Real Coinbase prices
- Claude AI assistant
- $20/month subscription

The crypto bot market is $1.2B and growing 35% annually. Our edge: simplest UX in the market, cheapest price ($20 vs $50+ competitors), and a money-back guarantee.

I'm raising $150K CAD pre-seed to fund marketing and multi-exchange support.

Would you be open to a 15-minute call?

[Your Name]
aix.8bit.ca

---

## IMPORTANT: What Investors Want to See

1. **Live product** ✅ (you have this)
2. **Users** — get at least 50-100 signups before pitching
3. **Revenue** — even $200/month MRR makes you fundable
4. **Market size** — $1.2B bot market, 580M crypto owners
5. **Your story** — why you, why now, why this

The #1 thing that gets funding: **traction**. Get users first, then investors chase you.
